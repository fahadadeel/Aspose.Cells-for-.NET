...
This directory contains the automated documentation generation system for Aspose.Cells .NET Examples.

## Overview

The system automatically detects new or modified examples in the `Examples/CSharp` directory and generates comprehensive documentation using AI analysis. The generated documentation is then automatically submitted as pull requests to the [Aspose.Cells Documentation repository](https://github.com/aspose-cells/Aspose.Cells-Documentation).
...
- Push to `main` or `develop` branches with changes in `Examples/CSharp/**/*.cs`
- Pull requests with changes to example files

### 2. Example Detection
- Analyzes git diff to find changed C# files
- Filters for actual example files (excludes test files)
- Focuses on examples in the CSharp project directory
...
2. **`DOCUMENTATION_REPO_TOKEN`** - GitHub token with access to documentation repo
   ...
   - For repository: `aspose-cells/Aspose.Cells-Documentation`
...
keywords: "Relevant, SEO, Keywords, Aspose.Cells, .NET"
...
python3 generate_docs.py \
  --example-file "Examples/CSharp/SomeExample.cs" \
  --examples-repo "../../" \
...
1. **No examples detected**
   - Check that files are in `Examples/CSharp` directory
   - Verify files follow naming conventions (not test files)
   - Ensure changes are properly committed
