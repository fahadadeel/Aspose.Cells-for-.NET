...
class ExampleValidator {
    ...
    validateStructure(filePath) {
        ...
        // Check for required elements (may vary for Aspose.Cells)
        // License check is optional for Aspose.Cells examples
        
        if (!content.match(/public\s+static.*Run/)) {
            throw new Error(`Example must have a public static Run method: ${filePath}`);
        }
        
        const expectedNamespace = `Aspose.Cells.Examples.CSharp.${category}`;
        const alternativeNamespace = `CSharp.${category}`;
        if (!content.includes(`namespace ${expectedNamespace}`) && 
            !content.includes(`namespace ${alternativeNamespace}`) &&
            !content.includes(`namespace Aspose.Cells.Examples.CSharp`)) {
            console.log(`⚠️ Warning: Example not in expected namespace ${expectedNamespace}: ${filePath}`);
        }
...
using System.Linq;
using Aspose.Cells.Examples.CSharp.${exampleInfo.category};
...
class TestRunner 
{
     ...
     */
    generateProjectFile() {
        // Use absolute path to Examples CSharp project
        const examplesCSharpProject = path.resolve(REPO_ROOT, 'Examples', 'CSharp', 'CSharp.csproj');
  ...
  <ItemGroup>
    <ProjectReference Include="${examplesCSharpProject}" />
  </ItemGroup>
</Project>`;
    }
